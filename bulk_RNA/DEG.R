...
zero_length_and_unexpressed = apply(rcm$length, 1, min) == 0
rcm$length = rcm$length[!zero_length_and_unexpressed,]
rcm$abundance = rcm$abundance[!zero_length_and_unexpressed,]
rcm$counts = rcm$counts[!zero_length_and_unexpressed,]
...
for (idx in 1:nrow(groupmeta)) {
  ...
  if (nrow(dematrix) == 0){
    ...
    if (!(is.null(kegg_enrich) || nrow(kegg_enrich@result)==0)){
      ...
      ggplot2::ggsave(paste(opt$outdir, compare_name, 'dotplot.pdf', sep='/'), width = 8, height = 10)
      ### heatplot
      heatplot(kegg_enrich, foldChange=gene_list)
      ggplot2::ggsave(paste(opt$outdir, compare_name, 'heatplot.pdf', sep='/'), width = 15, height = 3)
      ### cnetplot
      cnetplot(kegg_enrich)
      ggplot2::ggsave(paste(opt$outdir, compare_name, 'cnetplot.pdf', sep='/'), width = 10, height = 10)
    }
    ...
    for ( lev in c('BP','CC','MF')) {
      ...
      if (!(is.null(go_enrich) || nrow(go_enrich@result)==0)){
        ...
        ### dot plot
        dotplot(kegg_enrich, showCategory=30)
        ggplot2::ggsave(paste(opt$outdir, compare_name, paste0(lev,'dotplot.pdf'), sep='/'), width = 8, height = 10)
	### heatplot
        heatplot(kegg_enrich, foldChange=gene_list)
        ggplot2::ggsave(paste(opt$outdir, compare_name, paste0(lev,'heatplot.pdf'), sep='/'), width = 15, height = 3)
	### cneplot
        cnetplot(kegg_enrich)
	ggplot2::ggsave(paste(opt$outdir, compare_name, paste0(lev,'cnetplot.pdf'), sep='/'), width = 10, height = 10)
      }
    }
  }
}
